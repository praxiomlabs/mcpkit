...
fn generate_server_handler(
    ...
    let mut capability_chain = vec![quote!(::mcp_core::capability::ServerCapabilities::new())];
    ...
    let capabilities = if capability_chain.len() == 1 {
        quote!(::mcp_core::capability::ServerCapabilities::new())
    } else {
    ...
    quote! {
        impl ::mcp_server::ServerHandler for #self_ty {
            fn server_info(&self) -> ::mcp_core::capability::ServerInfo {
                ::mcp_core::capability::ServerInfo::new(#name, #version)
            }

            fn capabilities(&self) -> ::mcp_core::capability::ServerCapabilities {
                #capabilities
            }
...
fn generate_tool_handler(tools: &[ToolMethod], self_ty: &syn::Type) -> TokenStream {
    ...
    quote! {
        impl ::mcp_server::ToolHandler for #self_ty {
            fn list_tools(
                &self,
                _ctx: &::mcp_server::Context,
            ) -> impl std::future::Future<Output = Result<Vec<::mcp_core::types::Tool>, ::mcp_core::error::McpError>> + Send {
                async move {
                    Ok(vec![
                        #(#tool_defs),*
                    ])
            ...
            fn call_tool(
                ...
                _ctx: &::mcp_server::Context,
            ) -> impl std::future::Future<Output = Result<::mcp_core::types::ToolOutput, ::mcp_core::error::McpError>> + Send {
                ...
                async move {
                    ...
                    match name {
                        #(#dispatch_arms)*
                        _ => Err(::mcp_core::error::McpError::method_not_found_with_suggestions(
                            name,
...
fn generate_resource_handler(resources: &[ResourceMethod], self_ty: &syn::Type) -> TokenStream {
    ...
    quote! {
        impl ::mcp_server::ResourceHandler for #self_ty {
            fn list_resources(
                &self,
                _ctx: &::mcp_server::Context,
            ) -> impl std::future::Future<Output = Result<Vec<::mcp_core::types::Resource>, ::mcp_core::error::McpError>> + Send {
                async move {
                    Ok(vec![
                        #(#resource_defs)*
                    ])
            ...
            fn read_resource(
                ...
                uri: &str,
                _ctx: &::mcp_server::Context,
            ) -> impl std::future::Future<Output = Result<Vec<::mcp_core::types::ResourceContents>, ::mcp_core::error::McpError>> + Send {
                ...
                async move {
                    ...
                    Err(::mcp_core::error::McpError::resource_not_found(uri))
                }
            }
        }
    }
...
fn generate_prompt_handler(prompts: &[PromptMethod], self_ty: &syn::Type) -> TokenStream {
                    ...
                    if param.is_optional {
                        ...
                        quote! {
                            let #name: #ty = {
                                ...
                                {
                                    Some(v) => v.clone(),
                                    None => return Err(::mcp_core::error::McpError::invalid_params(
                                        #prompt_name,
                                ...
                                match ::serde_json::from_value::<#ty>(value) {
                                    Ok(v) => v,
                                    Err(e) => return Err(::mcp_core::error::McpError::invalid_params(
                                        #prompt_name,
    ...
    quote! {
        impl ::mcp_server::PromptHandler for #self_ty {
            fn list_prompts(
                &self,
                _ctx: &::mcp_server::Context,
            ) -> impl std::future::Future<Output = Result<Vec<::mcp_core::types::Prompt>, ::mcp_core::error::McpError>> + Send {
                async move {
                    Ok(vec![
                        #(#prompt_defs),*
                    ])
            ...
            fn get_prompt(
                ...
                _ctx: &::mcp_server::Context,
            ) -> impl std::future::Future<Output = Result<::mcp_core::types::GetPromptResult, ::mcp_core::error::McpError>> + Send {
