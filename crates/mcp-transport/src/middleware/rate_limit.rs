...
//!
//! ```ignore
//! use mcp_transport::middleware::{LayerStack, RateLimitLayer, RateLimitConfig};
...
//!     .with(RateLimitLayer::new(config))
//!     .into_inner();
//! ```
...
impl RateLimiter {
        ...
        if allowed {
            ...
            match self.config.on_limit {
                ...
                RateLimitAction::Wait => {
                    // Wait for token refill and retry (non-recursive via loop)
                    #[cfg(feature = "tokio-runtime")]
                    {
                        ...
                        // Retry after waiting (loop back rather than recurse)
    ...
    async fn check_token_bucket(&self) -> bool {
        ...
        #[cfg(not(feature = "tokio-runtime"))]
        let mut last_refill = self.state.last_refill.lock().unwrap();
    ...
    async fn check_sliding_window(&self) -> bool {
        ...
        #[cfg(not(feature = "tokio-runtime"))]
        let mut times = self.state.request_times.lock().unwrap();
    ...
    pub async fn reset(&self) {
        ...
        #[cfg(not(feature = "tokio-runtime"))]
        {
            *self.state.last_refill.lock().unwrap() = Instant::now();
            *self.state.window_start.lock().unwrap() = Instant::now();
            self.state.request_times.lock().unwrap().clear();
        }
    }
}
