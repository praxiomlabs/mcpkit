...
//!
//! ```ignore
//! use mcp_transport::unix::{UnixTransport, UnixListener};
//!
//! // Server-side
//! let listener = UnixListener::bind("/tmp/mcp.sock").await?;
//! let transport = listener.accept().await?;
//!
//! // Client-side
//! let transport = UnixTransport::connect("/tmp/mcp.sock").await?;
//! ```
