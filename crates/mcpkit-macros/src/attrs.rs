...
pub struct ToolAttrs {
    ...
    /// Maximum input size in bytes (for security limiting).
    ///
    /// When set, the tool will reject inputs larger than this size.
    /// This is a security feature to prevent denial-of-service attacks
    /// via excessively large inputs.
    ///
    /// # Example
    ///
    /// ```ignore
    /// #[tool(description = "Process text", max_input_size = 1024 * 1024)]
    /// async fn process(&self, text: String) -> ToolOutput {
    ///     // Input is guaranteed to be <= 1MB
    ///     ToolOutput::text(text.len().to_string())
    /// }
    /// ```
    #[darling(default)]
