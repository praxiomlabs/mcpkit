...
//!
//! ```ignore
//! use mcp_server::{ServerBuilder, ServerHandler, ServerRuntime};
//! use mcp_transport::stdio::StdioTransport;
//!
//! let server = ServerBuilder::new(MyHandler).build();
//! ServerRuntime::new(server, transport).run().await?;
//! ```
...
use mcp_core::protocol::{Message, Notification, ProgressToken, Request, Response};
...
impl ServerState {
    ...
    pub fn client_caps(&self) -> ClientCapabilities {
        self.client_caps.read().unwrap().clone()
    }
    ...
    Tr::Error: Into<McpError>,
{
    ...
    async fn handle_initialize(
        ...
        let result = serde_json::json!({
            "protocolVersion": mcp_core::capability::PROTOCOL_VERSION,
            "serverInfo": {
                "name": "mcp-server",
                "version": "1.0.0"
            },
