...
// Our SDK types
use mcp_core::protocol::{Message as OurMessage, Request as OurRequest};
use mcp_core::types::Tool as OurTool;
...
fn bench_request_serialization_comparison(c: &mut Criterion) {
    ...
    group.bench_function("rust_mcp_sdk", |b| {
...
fn bench_message_parsing_comparison(c: &mut Criterion) {
    ...
    group.bench_function("rust_mcp_sdk_request", |b| {
        b.iter(|| serde_json::from_str::<OurMessage>(black_box(request_json)).unwrap())
    });

    group.bench_function("rust_mcp_sdk_response", |b| {
        b.iter(|| serde_json::from_str::<OurMessage>(black_box(response_json)).unwrap())
    });

    group.bench_function("rust_mcp_sdk_notification", |b| {
...
fn bench_tool_operations_comparison(c: &mut Criterion) {
    ...
    group.bench_function("rust_mcp_sdk_tool_creation", |b| {
        b.iter(|| {
...
fn bench_payload_sizes_comparison(c: &mut Criterion) {
    ...
    for size in [100, 1000, 10000].iter() {
        ...
        group.bench_with_input(
            BenchmarkId::new("rust_mcp_sdk_serialize", size),
            &our_request,
...
fn bench_tool_call_params_comparison(c: &mut Criterion) {
    ...
    group.bench_function("rust_mcp_sdk_serialize", |b| {
    ...
    group.bench_function("rust_mcp_sdk_deserialize", |b| {
