...
//! let conn: Connection<Disconnected> = Connection::new(transport);
//!
//! // Connect to get a Connected connection
//! let conn: Connection<Connected> = conn.connect().await?;
//!
//! // Initialize to get Initializing
//! let conn: Connection<Initializing> = conn.initialize(params).await?;
//!
//! // Complete initialization to get Ready
//! let conn: Connection<Ready> = conn.complete().await?;
//!
//! // Now we can send requests
//! let response = conn.request(my_request).await?;
//!
//! // Shutdown when done
//! let conn: Connection<Closing> = conn.shutdown().await?;
//! ```
