//! State management for MCP Warp integration.
...
mod tests {
    ...
    impl ToolHandler for TestHandler {
        async fn list_tools(&self, _ctx: &Context<'_>) -> Result<Vec<Tool>, McpError> {
            Ok(vec![Tool::new("warp-tool").description("A warp test tool")])
        }
        ...
        async fn call_tool(
            ...
            Ok(ToolOutput::text("warp result"))
        }
    }
    ...
    impl ResourceHandler for TestHandler {
        ...
        async fn read_resource(
            ...
            Ok(vec![ResourceContents::text(uri, "warp content")])
        }
    }
    ...
    fn test_mcp_state_creation() {
        ...
        assert_eq!(state.server_info.name, "test-warp-server");
    ...
    fn test_has_server_info_trait() {
        ...
        assert_eq!(info.name, "test-warp-server");
