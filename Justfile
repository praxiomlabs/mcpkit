...
generate-proto:
    ...
    GENERATED=$(find target/debug/build/mcpkit-transport-*/out -name "mcp.rs" 2>/dev/null | head -1)
    if [ -z "$GENERATED" ]; then
        printf '{{red}}[ERR]{{reset}}  Generated file not found. Build may have failed.\n'
        exit 1
    fi

    DEST="crates/mcpkit-transport/src/grpc/mcp_proto.rs"

    printf '{{cyan}}[INFO]{{reset}} Found generated file: %s\n' "$GENERATED"
    printf '{{cyan}}[INFO]{{reset}} Updating %s...\n' "$DEST"

    # Create new file with header using printf to avoid Just heredoc parsing issues
    {
    ...
    printf '{{cyan}}[INFO]{{reset}} Formatting generated code...\n'
    {{cargo}} fmt -p mcpkit-transport

    printf '{{green}}[OK]{{reset}}   Protobuf code regenerated at %s\n' "$DEST"
